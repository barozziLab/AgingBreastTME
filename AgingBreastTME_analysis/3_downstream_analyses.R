...
# A collection of all conducted downstream analyses and visualizations on this dataset.
# Additionally to plots shown in the publication, this script also contains several 
# exploratory analyses and plots that are not reported in the publication.

# set idents

for (so_name in names(so.split.processed)) {
  
  Idents(so.split.processed[[so_name]]) <- "cluster_label"
  so.split.processed[[so_name]]@active.ident <- factor(so.split.processed[[so_name]]@active.ident, levels = ca$cluster_label)
...
for (so_name in names(so.split.processed)) {
  ...
  if (so_name == "epcam+") {
    Idents(so.split.processed[[so_name]]) <- "seurat_clusters"
    umap <- UMAPPlot(so.split.processed[[so_name]], label = T, label.box = TRUE)
    ...
    Idents(so.split.processed[[so_name]]) <- "cluster_label"
    umap <- UMAPPlot(so.split.processed[[so_name]], group.by = "cluster_label", label = T,
                     cols  = col[so.split.processed[[so_name]][[]] %>% pull(cluster_label) %>% unique()])
    
    Idents(so.split.processed[[so_name]]) <- "seurat_clusters"
    umap_lab <- UMAPPlot(so.split.processed[[so_name]], label = T, label.box = TRUE, group.by = "seurat_clusters", 
...
markers_epcam <- read_tsv("../data/markers_epcam_soupx.tsv")
...
Idents(so.split.processed[["epcam+"]]) <- "cluster_label"
dp_combined_filtered <- custom_dotplot(so = so.split.processed[["epcam+"]], 
...
dp_small_cl_markers_adapted <- custom_dotplot(so = so.split.processed[["epcam+"]], 
                                              features = mg_small, 
                                              color_pal = "Greys")
...
########################
out_folder <- "results/stromal"
if(!dir.exists(out_folder)) dir.create(out_folder)
  
#Figure 4A (top2 non-Haemoglobin markers in each stromal cluster) and S4A
stromal_markers <- read_tsv("../data/markers_stromal_soupx.tsv")
...
Idents(so.split.processed[["stromal"]]) <- "cluster_label"
fig4a <- custom_dotplot(so = so.split.processed[["stromal"]],
...
stromal_minimal <- read_xlsx("../data/Stromal_Cell_Markers_Minimal.xlsx")

Idents(so.split.processed[["stromal"]]) <- "cluster_label"
fig4a_minimal <- custom_dotplot(so = so.split.processed[["stromal"]],
...
stromal_comm_markers <- read_tsv("../data/markers_stromal_soupx.tsv")
...
coll_markers_dp <- custom_dotplot(so = so.split.processed[["stromal"]], 
...
clus <- so.split.processed[["stromal"]]@meta.data$cluster_label %>% unique()
clus_keep <- clus[!grepl(pattern = "Granulocytes|Contaminated|Erythrocytes|Basophils|Pro-inflammatory", x = clus)]
so.stromal.clear <- subset(x = so.split.processed[["stromal"]],
...
vln_col <- VlnPlot(object = so.split.processed[["stromal"]], features = coll_markers)
dot_col <- custom_dotplot(so = so.split.processed[["stromal"]],
                          features = coll_markers,
                          assay = "SCT",
                          color_pal = "Reds")
vln_col_age <- VlnPlot(object = so.split.processed[["stromal"]], features = coll_markers, split.by = "age", split.plot = TRUE) +
  plot_annotation(title = 'Collagen expression in stroma, split by age')
dot_col_age <- custom_dotplot(so = so.split.processed[["stromal"]],
                              features = coll_markers,
                              assay = "SCT",
                              split.by = "age",
                              color_pal = "Reds") +
...
vln_col_tm <- VlnPlot(object = so.split.processed[["stromal"]], features = coll_markers, split.by = "treatment", split.plot = TRUE) +
  plot_annotation(title = "Collagen expression in stromal clusters, split by treatment")
dot_col_tm <- custom_dotplot(so = so.split.processed[["stromal"]],
                              features = coll_markers,
                              assay = "SCT",
                              split.by = "treatment",
                              color_pal = "Reds") +
...
# module score
so.split.processed[["stromal"]] <- AddModuleScore(object = so.split.processed[["stromal"]],
                                             features = list(coll_markers),
                                             ctrl = 5,
                                             name = "total_collagens")

vln_col_module <- VlnPlot(so.split.processed[["stromal"]], features = "total_collagens1", split.by = "age", split.plot = T)
...
so.stromal.CAFs <- subset(x = so.stromal.filt, 
                          subset = seurat_clusters %in% all_CAFs$Cluster_number)
...
#######################################################
so.stromal <- so.split.processed[["stromal"]]
annotation <- read_xlsx(annotation_file) %>%
  mutate(Cluster_number = as.factor(Cluster_number)) %>%
  mutate(Cluster_Label = paste0(`Cell-type_label`, "_c", Cluster_number))
...
joined_meta <- obj_meta %>% 
  left_join(annotation_stromal, by = c("seurat_clusters" = "Cluster_number")) %>%
...
out_folder <- "results/immune"
if(!dir.exists(out_folder)) dir.create(out_folder)

# 1. Pooling all T-cells
so.tcells <- subset(so.split.processed[["cd45+"]], subset = seurat_clusters %in% c(2, 4, 5, 7, 12, 13, 14, 16, 19, 20))
...
#DEG analyses old versus young on cell type label level
#######################################################
so.immune <- so.split.processed[["cd45+"]]
annotation <- read_xlsx(annotation_file) %>%
  mutate(Cluster_number = as.factor(Cluster_number)) %>%
  mutate(Cluster_Label = paste0(`Cell-type_label`, "_c", Cluster_number))
...
joined_meta <- obj_meta %>% 
  left_join(annotation_immune, by = c("seurat_clusters" = "Cluster_number")) %>%
  column_to_rownames("barcodes")
...
for (compartment in c("cd45", "epcam", "stromal")) {
  markers[[compartment]] <- read_tsv(paste0("../data/markers_", compartment, "_soupx.tsv")) %>%
...
ggsave(filename = file.path("results", "number_pct_diff_markers_per_cluster.pdf"), 
       n_pct_markers_plt,
       width = 10, height = 3)

#Panel B: Number of marker genes (up/down-regulated) across clusters, by compartment; boxplot
...
ggsave(filename = file.path("results", "n_markers_across_clusters.pdf"), 
       n_markers_boxplot_plt,
       width = 6, height = 3)
ggsave(filename = file.path("results", "n_markers_across_clusters_pct_diff_larger0.2.pdf"), 
